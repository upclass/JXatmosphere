...
//    public static final int CHART_HEIGHT=150;
     ...
     */
    private float minHigh, maxHigh;
    /**
     * 数据集合
     */
    private List<GdybBeen.DataBean> datas;
    /**
     * 是否是平滑曲线，默认折线
     */
    ...
    private void init(Context context) {
        ...
        linePaint.setStrokeWidth(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1.5f, getResources().getDisplayMetrics()));
        linePaint.setColor(Color.rgb(220, 160, 0));
        ...
        eachChartHeight = chartHeight - padding;
    }
    ...
    public void setDatas(List<GdybBeen.DataBean> datas) {
        this.datas = datas;

        lineSize = datas.size();

        minHigh = Float.valueOf(datas.get(0).getTemper());
        maxHigh = Float.valueOf(datas.get(0).getTemper());

        for (int i = 1; i < datas.size(); i++) {
            float fh = Float.valueOf(datas.get(i).getTemper());
            if (minHigh > fh) {
                minHigh = fh;
            }
            if (maxHigh < fh) {
...
//            float fl=datas.get(i).getLowTemperature();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < lineSize; i++) {
            ...
            float y = chartHeight - padding / 2 - (Float.valueOf(datas.get(i).getTemper()) - minHigh) * perHeightTop;
//            canvas.drawCircle(x, y, pointRaidus, pointPaint);
            ...
            if (isCubic) {
                if (Float.isNaN(currentPointX)) {
                    ...
                    currentPointY = chartHeight - padding / 2 - (Float.valueOf(datas.get(i).getTemper()) - minHigh) * perHeightTop;
                }
                if (Float.isNaN(previousPointX)) {
                    if (i > 0) {
                        previousPointX = eachWidth / 2f + (i - 1) * eachWidth;
                        previousPointY = chartHeight - padding / 2 - (Float.valueOf(datas.get(i - 1).getTemper()) - minHigh) * perHeightTop;
                    } else {
                ...
                if (Float.isNaN(prePreviousPointX)) {
                    if (i > 1) {
                        ...
                        prePreviousPointY = chartHeight - padding / 2 - (Float.valueOf(datas.get(i - 2).getTemper()) - minHigh) * perHeightTop;
                    } else {
                ...
                if (i < lineSize - 1) {
                    ...
                    nextPointY = chartHeight - padding / 2 - (Float.valueOf(datas.get(i + 1).getTemper()) - minHigh) * perHeightTop;
                } else {
            ...
            String label = datas.get(i).getTemper() + "°";
