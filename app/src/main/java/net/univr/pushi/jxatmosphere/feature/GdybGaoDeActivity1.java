...
import net.univr.pushi.jxatmosphere.adapter.GdybAdapter;
...
import net.univr.pushi.jxatmosphere.widget.FloatingItemDecoration;
...
public class GdybGaoDeActivity1 extends BaseActivity implements View.OnClickListener, DistrictSearch.OnDistrictSearchListener, AMap.OnMapClickListener, GeocodeSearch.OnGeocodeSearchListener {
    ...
    GdybAdapter gdybAdapter;
    String tempRiqi="";


    @Override
    public int getLayoutId() {
        return R.layout.activity_gdyb_gao_de1;
    }
...
//                            mAMap.addMarker(markerOption);
    ...
    public GdybAdapter getGdybAdapter(){
        if(gdybAdapter==null){
            gdybAdapter=new GdybAdapter(context,data);
            final FloatingItemDecoration floatingItemDecoration=new FloatingItemDecoration(this, getResources().getColor(R.color.black_yj),1,1);
    ...
    private void getTestdata() {
        ...
        RetrofitHelper.getWeatherMonitorAPI()
                ...
                .subscribe(gdybBeen1 -> {
                    ...
                    initRetDataAndTitle(keys,data,oneH);
                    initRetDataAndTitle(keys,data,threeH);
                    initRetDataAndTitle(keys,data,twelveH);
                    initRetDataAndTitle(keys,data,twenty_fourH);
                    getGdybAdapter().setNewData(data);
    ...
    private void initRetDataAndTitle(Map<Integer, String> keys, List<GdybBeen1.DataBean.ContentBean> contentBeans, List<GdybBeen1.DataBean.ContentBean> hourData) {
        //用于判断是否同放一天
        for (int j = 0; j < hourData.size(); j++) {
            GdybBeen1.DataBean.ContentBean contentBean = hourData.get(j);
            String forecastTime = contentBean.getForecastTime();
            ...
            String riqi = calendar.get(Calendar.YEAR)+"年"+(calendar.get(Calendar.MONTH)+1)+"月"+calendar.get(Calendar.DAY_OF_MONTH)+"日";
            String dayHour = String.valueOf(calendar.get(Calendar.HOUR_OF_DAY))+":00";
            contentBean.setForecastTime(dayHour);

            if(!riqi.equals(tempRiqi))  keys.put(contentBeans.size(),riqi);//日期不同加入悬浮头部
                tempRiqi=riqi;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.search_info:
                ((InputMethodManager) getSystemService(INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(GdybGaoDeActivity1.this.getCurrentFocus().getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);
