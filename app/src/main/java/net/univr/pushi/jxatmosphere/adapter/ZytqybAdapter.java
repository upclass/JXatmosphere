...
public class ZytqybAdapter extends BaseQuickAdapter <ZytqybBeen.DataBean.InfoArrayBean,BaseViewHolder> {
    public ZytqybAdapter( @Nullable List<ZytqybBeen.DataBean.InfoArrayBean> data) {
        super(R.layout.zytqyb_recycle_item, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, ZytqybBeen.DataBean.InfoArrayBean item) {
...
//        }
        ((TextView) helper.getView(R.id.item_1)).setText(item.getInfo_key());
        ((TextView) helper.getView(R.id.item_2)).setText(item.getInfo_values());
    }
